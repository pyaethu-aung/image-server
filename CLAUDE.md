...
**Implementation steps 1 to 4 complete.** The compose stack (Postgres 18, Redis 8, server image with libvips), migrations, sqlc setup, config package, the `Storage` interface with its local filesystem backend (`internal/storage/`), the upload path, and the transform-on-read path are built and tested: `POST /v1/images` (multipart) and `POST /v1/images/from-url` (SSRF-guarded via `internal/fetch/`), magic-byte detection and the bomb guard (`internal/imageproc/`), `GET /v1/images/{id}` (original, or transformed via libvips using the `w`/`h`/`fmt`/`q`/`fit` params) and `GET /v1/images/{id}/meta`, API-key auth + Redis rate-limit middleware, DB/Redis wiring in `main.go`, and the spec-validated `apitest` harness. Step 5 (derivative caching) is not built yet: `GET` regenerates every transform on demand with no cache, and `DELETE /v1/images/{id}` returns 501 via the generated `Unimplemented` embed. This document is the implementation contract; update it as reality diverges from the plan.
